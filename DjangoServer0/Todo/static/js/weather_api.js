...
const getFullHTML = (temp) => {
  var svg;
  if(temp < 0) svg = coldSvg;
  else if(temp > 20) svg = hotSvg;
  else svg = moderateSvg
  return `<div style="display: flex; align-items: baseline">
    <span style="margin: 0">
      ${svg}
    </span>
    <h1 style="margin-left: 30px">${temp}°C</h1>
  </div>`
}

const getTempFromProxy = async() => {
  const pro = await fetch('http://127.0.0.1:6999/gettemp/');
  const res = await pro.json();
  const temp = res.temp
  return temp
}

const loadTempHTML = (temp) => {
  const jbt = document.querySelector('#jbt')
  jbt.innerHTML = getFullHTML(temp)
}

((async() => {
  loadTempHTML(await getTempFromProxy())
})())
